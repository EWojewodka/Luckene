...
public class LuckeneDirectoryConfiguration {
	...
	public LuckeneDirectoryConfiguration init(String rootLocation) throws IOException {
		if (this.isInitialized) return this;
		this.isInitialized = true;
		
		this.directory = new SimpleFSDirectory(Paths.get(new File(rootLocation, this.code).getPath()));
		this.indexWriter = new IndexWriter(directory, new IndexWriterConfig(buildAnalyzer()));
		
		return this;
	}
	
	public static LuckeneDirectoryConfigurationBuilder builder(String code) {
		return new LuckeneDirectoryConfigurationBuilder()
			.code(code);
	}
